...
# This notebook comes from [KOJIMAR](https://www.kaggle.com/code/kojimar/fb3-single-pytorch-model-train/notebook)
#
# [Inference](https://www.kaggle.com/code/mozattt/fgm-fb3-single-pytorch-model-inference) too.

# If you have time, please check my other notebooks.
#
# * Inference : https://www.kaggle.com/code/mozattt/fgm-fb3-single-pytorch-model-inference

# * In this notebook, I started AWP.
# * Attention Pooling was used instead of mean Pooling.
# * AWP is buggy and may does not work, so I deleted it.
# * The batchsize cannot be set to 16 or more.(On a v3base basis, I've only used 8. Using 16 causes memory to explode.)
# * Using other models if run on Kaggle may result in insufficient memory or timeouts, try it yourself for details.
...
class CFG:
    ...
    num_cycles = 0.5
    num_warmup_steps = 0
    encoder_lr = 2e-5 * factor
    decoder_lr = 2e-5 * factor
    min_lr = 1e-6 * factor
    ...
    submission_file = (
        "../data/feedback-prize-english-language-learning/sample_submission.csv"
    )
...
if CFG.train:
    ...
    CFG.OUTPUT_DIR = f"./{CFG.identifier}/"  # type: ignore
    ...
    Fold = MultilabelStratifiedKFold(
        n_splits=CFG.n_fold, shuffle=True, random_state=CFG.cv_seed
    )
    for n, (train_index, val_index) in enumerate(
        Fold.split(CFG.df_train, CFG.df_train[CFG.target_cols])
    ):
...
if CFG.debug:
    ...
    CFG.trn_fold = [0]
    if CFG.train:
        CFG.df_train = CFG.df_train.sample(n=100, random_state=CFG.seed).reset_index(
            drop=True
        )
    ...
    def __init__(self, CFG, config_path=None, pretrained=False):
        ...
        if config_path is None:
            self.config = AutoConfig.from_pretrained(
                CFG.model, ouput_hidden_states=True
            )
...
def train_loop(folds, fold):
    ...
    def get_optimizer_grouped_parameters(
        model, layerwise_lr, layerwise_weight_decay, layerwise_lr_decay
    ):
    ...
    if CFG.llrd:
        ...
        grouped_optimizer_params = get_optimizer_grouped_parameters(
            model, CFG.layerwise_lr, CFG.layerwise_weight_decay, CFG.layerwise_lr_decay
        )
        optimizer = AdamW(
    ...
    for epoch in range(CFG.epochs):
        ...
        if CFG.save_all_models:
            torch.save(
                {"model": model.state_dict(), "predictions": predictions},
                CFG.OUTPUT_DIR
                + f"{CFG.model.replace('/', '-')}_fold{fold}_epoch{epoch + 1}.pth",
            )
    ...
    return (
        ...
        best_val_loss,
        valid_folds,
        pd.concat([df_epoch, df_scores], axis=1),
    )
...
if CFG.train:
    ...
    for fold in range(CFG.n_fold):
        if fold in CFG.trn_fold:
            best_train_loss, best_val_loss, _oof_df, df_epoch_scores = train_loop(
                CFG.df_train, fold
            )
            ...
            df_epoch_scores = df_epoch_scores[
                [
                    "file",
                    "model",
                    "cv_seed",
    ...
    LOGGER.info(f"========== CV ==========")
    _output_log = get_result(
        oof_df, "OOF", np.mean(train_loss_list), np.mean(val_loss_list)
    )
